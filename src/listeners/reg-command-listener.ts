...
class RegCommandListener extends BaseListener {
  ...
  protected async handleEvent(
    ...
    if (!accountDto.disabled) {
      ...
      await bot.sendMessage(message.chat.id, `Пользователь уже был добавлен! <b>(${activeUsers}/${totalUsers})</b>`, { parse_mode: 'HTML' });
      return;
    }
    ...
    await bot.sendMessage(message.chat.id, `Пользователь добавлен! <b>(${activeUsers + 1}/${totalUsers})</b>`, { parse_mode: 'HTML' });
